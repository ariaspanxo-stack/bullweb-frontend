...
export const TabDescuentos = ({ sales, isLoading, hasSearched }: Props) => {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Contenido normal â€” solo si hay datos o ya se buscÃ³ */}
      ...
      {/* Resumen */}
      <div className="bg-red-500/10 p-6 rounded-lg border-l-4 border-red-500 max-w-md">
        <p className="text-sm text-gray-400 mb-1">Total Descuentos Aplicados</p>
      ...
      <div className="bg-gray-900 rounded-lg shadow-sm overflow-hidden">
        {discounts.length === 0 ? (
          <div className="p-12 text-center text-gray-400">
            <p className="text-lg">Sin descuentos en el perÃ­odo seleccionado</p>
          </div>
        ) : (
        <table className="w-full">
          <thead className="bg-gray-800/40 border-b">
            <tr>
              ...
              <th className="px-4 py-3 text-left text-xs font-semibold text-gray-400 uppercase">Orden</th>
              <th className="px-4 py-3 text-left text-xs font-semibold text-gray-400 uppercase">Mesa</th>
              <th className="px-4 py-3 text-left text-xs font-semibold text-gray-400 uppercase">Tipo</th>
              <th className="px-4 py-3 text-left text-xs font-semibold text-gray-400 uppercase">Autorizado por</th>
          ...
          <tbody className="divide-y divide-white/10">
            {discounts.map((sale) => (
              <tr key={sale.id} className="hover:bg-white/5">
                <td className="px-4 py-3 text-sm text-gray-300">{formatDateTime(sale.createdAt)}</td>
                <td className="px-4 py-3 text-sm text-gray-300 font-medium">{sale.saleNumber}</td>
                <td className="px-4 py-3 text-sm text-gray-300">{sale.tableNumber || 'â€”'}</td>
                <td className="px-4 py-3">
                  <span className="px-3 py-1 rounded text-sm font-medium bg-blue-500/15 text-blue-400">
                    {formatType(sale.discountType)}
                  </span>
                </td>
                <td className="px-4 py-3 text-sm text-gray-400 font-medium">{sale.waiterName}</td>
                <td className="px-4 py-3 text-sm text-right font-semibold text-red-400">
                  {sale.discountType === 'PERCENTAGE' && (sale as any).discountValue
                    ? `${(sale as any).discountValue}% â†’ -${formatCurrency(sale.discount)}`
