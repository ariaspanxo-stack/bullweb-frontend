...
function resolveConfig(name: string): { color: string; bgColor: string; icon: any } {
  ...
  if (n.includes('efectivo') || n.includes('cash'))
    return { color: 'text-green-400',  bgColor: 'bg-green-500',  icon: Banknote };
  if (n.includes('débito') || n.includes('debito') || n.includes('debit'))
    return { color: 'text-cyan-400',   bgColor: 'bg-cyan-500',   icon: CreditCard };
  if (n.includes('crédito') || n.includes('credito') || n.includes('credit'))
    return { color: 'text-blue-400',   bgColor: 'bg-blue-500',   icon: CreditCard };
  if (n.includes('tarjeta') || n.includes('card'))
    return { color: 'text-blue-400',   bgColor: 'bg-blue-500',   icon: CreditCard };
  if (n.includes('transfer'))
    return { color: 'text-purple-400', bgColor: 'bg-purple-500', icon: Building2  };
  if (n.includes('cheque') || n.includes('check'))
    return { color: 'text-violet-400', bgColor: 'bg-violet-500', icon: Building2  };
  if (n.includes('cta') || n.includes('cuenta') || n.includes('corriente'))
    return { color: 'text-indigo-400', bgColor: 'bg-indigo-500', icon: Building2  };
  if (n.includes('voucher') || n.includes('vale'))
    return { color: 'text-orange-400', bgColor: 'bg-orange-500', icon: CreditCard };
  if (n.includes('fudo') || n.includes('yape') || n.includes('plin') || n.includes('digital'))
    return { color: 'text-pink-400',   bgColor: 'bg-pink-500',   icon: Smartphone };
  // método desconocido / personalizado
  return { color: 'text-gray-400',   bgColor: 'bg-gray-500',   icon: CreditCard };
}
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between mb-2">
        <h4 className="text-sm font-semibold text-gray-300">Desglose de Pagos</h4>
      ...
      {paymentData.map((payment, index) => {
        ...
        const bgLight = payment.bgColor.replace('-500', '-500/20');
      ...
      <div className="pt-2 border-t border-white/10 flex items-center justify-between">
        <span className="text-sm font-semibold text-gray-300">Total Pagado</span>
        <span className="text-base font-bold text-white tabular-nums">${totalPaid.toLocaleString('es-CL')}</span>
      </div>

      {sale.total > totalPaid && (
        <div className="flex items-center justify-between text-sm">
          <span className="text-orange-400 font-medium">Pendiente</span>
          <span className="text-orange-400 font-bold tabular-nums">${(sale.total - totalPaid).toLocaleString('es-CL')}</span>
        </div>
      )}

      <div className="pt-2 grid grid-cols-2 gap-2 text-xs">
        <div className="bg-gray-800/60 border border-white/5 rounded p-2">
          <div className="text-gray-400">Método principal</div>
          <div className="font-semibold text-white mt-0.5">{paymentData[0]?.method ?? 'N/A'}</div>
        </div>
        <div className="bg-gray-800/60 border border-white/5 rounded p-2">
          <div className="text-gray-400">Métodos usados</div>
          <div className="font-semibold text-white mt-0.5">{paymentData.length}</div>
        </div>
      </div>
    </div>
  );
