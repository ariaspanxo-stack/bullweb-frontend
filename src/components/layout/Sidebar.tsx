...
const menuSections: MenuSection[] = [
  ...
  },
  {
    ...
    items: [
      ...
      // Mapeo Delivery oculto temporalmente del menú (mismo patrón que /apps).
      // La ruta /delivery/mappings queda accesible solo por URL directa.
    ...
    <>
      ...
      <aside
        className={cn(
          ...
          'w-[220px] bg-gray-950 border-r border-white/5',
      ...
      >
          ...
          <div className="flex items-center gap-2.5 px-4 py-3 flex-1">
            <div className="flex items-center gap-2">
              ...
              <div className="leading-tight">
                <span className="text-white font-bold text-sm">BullWeb</span>
        ...
        <nav className="flex-1 overflow-y-auto py-1 px-1 [&::-webkit-scrollbar]:w-0">
          <div>
            {visibleSections.map((section, sIdx) => (
              <div key={section.title}>
                <div className={cn('px-3 mb-1', sIdx === 0 ? 'pt-3' : 'mt-6')}>
                  <p className="text-[10px] font-semibold uppercase tracking-wider text-gray-600 whitespace-nowrap">
                    {section.title}
                  </p>
                </div>
          ...
          <div className="border-t border-white/10 p-3 space-y-1">
            ...
            <div className="grid grid-cols-2 gap-1 px-1 pb-1">
              <Link
                to="/profile"
                onClick={() => window.innerWidth < 1024 && onClose()}
                className="flex items-center justify-center gap-1.5 text-xs text-gray-500 hover:text-white py-1.5 rounded-lg hover:bg-white/5 transition-colors"
              >
            ...
            <div className="flex items-center gap-2.5 px-2 py-1.5 rounded-xl bg-white/5">
              <div className="w-8 h-8 rounded-full bg-gradient-to-br from-brand-400 to-brand-600 flex items-center justify-center text-white text-xs font-bold flex-shrink-0 ring-2 ring-brand-500/30">
                {user.name?.[0]?.toUpperCase() ?? 'U'}
              </div>
              <div className="min-w-0 flex-1">
                <p className="text-white text-xs font-semibold leading-tight break-words">{user.name ?? 'Usuario'}</p>
                <p className="text-gray-500 text-[10px] truncate leading-tight">{user.role?.name ?? 'Sin rol'}</p>
              </div>
        ...
        <div className="px-3 pb-4 pt-1 border-t border-white/10">
          ...
          >
            <div className="flex items-center gap-3">
              <ShieldCheck className="h-4 w-4 flex-shrink-0 text-brand-400" />
